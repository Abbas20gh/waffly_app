...
│       ├── boxcode.ts              ← کد کوتاه ۵ رقمی ترتیبی جعبه (بدون معنا — v2.5.1)
...
| 3 | boxes | code (عدد ترتیبی کوتاه ۵ رقمی از v2.5.1 — کدهای قدیمی ۱۰ رقمی TTDDMMNNSS معتبرند), productionId, breadTypeId, count, date, hasEssence (0/1 — اسانس در سطح هر جعبه), essenceType? (مثل پرتقالی — لیست ثابت ESSENCE_TYPES در types.ts، قابل‌گسترش), note? — ایندکس code/date/productionId |
...
| **production** | ثبت تولید روزانه (نوع نان، تعداد کل، تعداد جعبه، تعداد در هر جعبه، ضایعات، انتقال از روز قبل با `carriedFrom`)، تولید خودکار جعبه‌ها با کد کوتاه ترتیبی (nextBoxSerial از کدهای موجود +۱) + **بخش اختیاری اسانس در فرم (طعم از ESSENCE_TYPES + تعداد جعبه اسانس‌دار)**، **بخش بازشدنی «جعبه‌های این تولید» در کارت هر تولید با ویرایش/حذف تک‌تک جعبه‌ها** (دیالوگ ویرایش: نوع نان، سوییچ اسانس‌دار + طعم، یادداشت — کد چاپی ثابت می‌ماند)، مصرف مواد |
...
- **کد جعبه کوتاه (v2.5.1)**: عدد ترتیبی ۵ رقمی `00001`…`99999` بدون هیچ معنایی — `nextBoxSerial(کدهای موجود)` بزرگ‌ترین سریِ کدهای ≤۵ رقمی +۱ (کدهای قدیمی ۱۰ رقمی نادیده/معتبر؛ بعد از ۹۹۹۹۹ کد ۶ رقمی و همچنان یکتا می‌شود)
