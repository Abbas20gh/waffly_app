...
│       ├── boxcode.ts              ← کد ۱۰ رقمی جعبه TTDDMMNNSS
...
- **کد جعبه ۱۰ رقمی `TTDDMMNNSS`**: TT=کد نوع نان (۲ رقم)، DD=روز شمسی، MM=ماه شمسی، NN=تعداد نان در جعبه، SS=شماره سری (اگر سری > ۹۹ شود کد بلندتر می‌شود تا یکتا بماند) — `parseBoxCode` برای خواندن
